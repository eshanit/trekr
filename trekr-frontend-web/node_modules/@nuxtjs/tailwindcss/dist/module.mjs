...
import { useNuxt, tryResolveModule, createResolver, addTemplate, findPath, resolveAlias, useLogger, isNuxt3, addDevServerHandler, isNuxt2, defineNuxtModule, resolvePath, getNuxtVersion, installModule, addVitePlugin } from '@nuxt/kit';
...
const logger$1 = useLogger("nuxt:tailwindcss");
const setupViewer = async (twConfig, config, nuxt = useNuxt()) => {
  ...
  nuxt.hook("listen", (_, listener) => {
    ...
    logger$1.info(`Tailwind Viewer: ${underline(yellow(withTrailingSlash(viewerUrl)))}`);
  });
};
const exportViewer = async (pathToConfig, config, nuxt = useNuxt()) => {
  ...
  nuxt.hook("nitro:build:public-assets", (nitro) => {
    ...
    logger$1.success(`Exported viewer to ${yellow(relative(nuxt.options.srcDir, dir))}`);
  });
};
